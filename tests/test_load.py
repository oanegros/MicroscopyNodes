...
def test_loading(axes_order, shape):
    data = np.random.rand(*shape) #seed this?
    xy_size = 0.5
    z_size = 1.5 
    ...
    #try if testing the actual operator + readout of metadata works

    tif_loader.load_tif(input_file, xy_scale, z_scale, axes_order)
    # test if dimensionality is correct
    # test if material is applied
    # test if scale bar is applied - get verts? 
    # test if transform locked


# def test_preset_env():
    # tif_loader.preset_environment()
